...
from campbellsciparser.devices import CampbellSCIBaseParser, UnknownPytzTimeZoneError
...
class BaseParserReadDataTest(ReadDataTestCase):

    def test_data_generator(self):
        l = [1, 2, 3]
        baseparser = CampbellSCIBaseParser()

        self.assertEqual(tuple(baseparser._data_generator(l)), (1, 2, 3))
...
class BaseParserTimeConvertingTest(unittest.TestCase):

    def test_datetime_to_string_no_time_zone(self):
        baseparser = CampbellSCIBaseParser()
        dt = datetime(2012, 11, 25, 22, 0, 0)
        expected_datetime = dt
        expected_datetime_string = str(expected_datetime)
        dt_string = baseparser._datetime_to_str_no_time_zone(dt=dt)
...
class CR10ParserExportDataTest(CR10ParserTestCase, ExportFileTestCase):
    ...
    def test_export_to_csv_file_created(self):
        ...
        cr10 = CR10Parser()
        data_mixed = cr10.read_mixed_data(infile_path=file)
        cr10.export_to_csv(data=data_mixed, outfile_path=output_file)
    ...
    def test_export_to_csv_file_content(self):
        ...
        output_file = os.path.join(THIS_DIR, 'testoutput/test.dat')
        cr10 = CR10Parser()
        data_source_file_mixed = cr10.read_mixed_data(infile_path=file)
        cr10.export_to_csv(data=data_source_file_mixed, outfile_path=output_file)
        data_exported_file_mixed = cr10.read_mixed_data(infile_path=output_file)

        self.assertTwoDataSetsLengthEqual(data_1=data_source_file_mixed, data_2=data_exported_file_mixed)
    ...
    def test_export_to_csv_file_headers(self):
        ...
        cr10.export_to_csv(data=data_source_file_mixed, outfile_path=output_file, export_headers=True)
        data_exported_file_mixed = cr10.read_mixed_data(infile_path=output_file)
...
class CR10ParserConvertTimeTest(ConvertTimeTest):
    ...
    def test_convert_time_from_data_row(self):
        file = os.path.join(THIS_DIR, 'testdata/cr10/csv_cr10_testdata_10_rows.dat')
        array_ids_info = {'201': 'label_1'}
        expected_datetime = datetime(2012, 11, 25, 22, 0, 0, tzinfo=pytz.UTC)
        cr10 = CR10Parser()
...
class HeadersTest(CR10ParserTestCase):
